...
IEvenNumber* pIEvenNumber = NULL;
IOddEvenNumberFactory* pIOddEvenNumberFactory = NULL;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	int iFirstEvenNumber, iNextEvenNumber;
	TCHAR str[255], strTemp[255];
	OLECHAR wszCLSID[] = OLESTR("clsid:B57E6B25-BB1F-4A75-A23D-20736CCC565A:");
	...
	case WM_CREATE:
		...
		StringFromCLSID(CLSID_OddEvenNumber, &szCLSID);
...
//		wsprintf(wszCLSID, TEXT("clsid:%s"), wszTemp);
		...
		hr = pIMoniker->BindToObject(pIBindCtx, NULL, IID_IOddEvenNumberFactory, (void**)&pIOddEvenNumberFactory);
		if (FAILED(hr))
		{
		...
		iFirstOddNumber = 57;
		iFirstEvenNumber = 56;
		hr = pIOddEvenNumberFactory->SetFirstOddEvenNumber(iFirstOddNumber, iFirstEvenNumber, &pIEvenNumber);
		if (FAILED(hr))
		{
			MessageBox(hwnd, TEXT("Failed to get IEvenNumber Interface Pointer"), TEXT("Error"), MB_OK);
			DestroyWindow(hwnd);
		}

		pIEvenNumber->GetNextEvenNumber(&iNextEvenNumber);

		wsprintf(str, TEXT("The Next Even Number From %2d Is %2d"), iFirstEvenNumber, iNextEvenNumber);
		MessageBox(hwnd, str, TEXT("Result"), MB_OK);

		pIEvenNumber->QueryInterface(IID_IOddNumber, (void**)&pIOddNumber);
		if (FAILED(hr))
		{
	...
	case WM_DESTROY:
		...
		if (pIEvenNumber)
		{
			pIEvenNumber->Release();
			pIEvenNumber = NULL;
		}

		if (pIOddEvenNumberFactory)
		{
			pIOddEvenNumberFactory->Release();
			pIOddEvenNumberFactory = NULL;
		}
