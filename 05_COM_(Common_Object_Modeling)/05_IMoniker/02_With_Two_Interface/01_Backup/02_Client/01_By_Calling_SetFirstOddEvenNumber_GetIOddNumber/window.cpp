...
//		wsprintf(wszCLSID, TEXT("clsid:%s"), wszTemp);
		...
		iFirstOddNumber = 57;
		iFirstEvenNumber = 56;
		hr = pIOddEvenNumberFactory->SetFirstOddEvenNumber(iFirstOddNumber, iFirstEvenNumber, &pIOddNumber);
		if (FAILED(hr))
		{
		...
		hr = pIOddNumber->QueryInterface(IID_IEvenNumber, (void**)&pIEvenNumber);
		if (FAILED(hr))
		{
			MessageBox(hwnd, TEXT("Failed to get iOddNumber Interface Pointer"), TEXT("Error"), MB_OK);
			DestroyWindow(hwnd);
