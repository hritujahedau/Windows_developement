...
//		wsprintf(wszCLSID, TEXT("clsid:%s"), wszTemp);
		...
		iFirstOddNumber = 57;
		iFirstEvenNumber = 56;
		hr = gpINumberFactory->SetFirstOddEvenNumber(iFirstOddNumber, iFirstEvenNumber, &gpIEvenNumber);
		if (FAILED(hr))
		{
		...
		iNextEvenNumber = 0;
		gpIEvenNumber->GetNextEvenNumber(&iNextEvenNumber);

		wsprintf(str, TEXT("The Next Even Number From %2d Is %2d"), iFirstEvenNumber, iNextEvenNumber);
		MessageBox(hwnd, str, TEXT("Result"), MB_OK);

		gpIEvenNumber->QueryInterface(IID_IOddNumber, (void**)&gpIOddNumber);
		if (FAILED(hr))
		{
		...
		gpIOddNumber->GetNextOddNumber(&iNextOddNumber);
