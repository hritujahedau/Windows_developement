...
//		wsprintf(wszCLSID, TEXT("clsid:%s"), wszTemp);
		...
		if (FAILED(hr))
		{
			MessageBox(hwnd, TEXT("Failed to get IOddNumber interface "), TEXT("ERROR"), MB_OK);
			DestroyWindow(hwnd);
		}

		if (gpIOddNumber == gpIOddNumberAgain)
		{
			MessageBox(hwnd, TEXT("Reflexivity TRUE"), TEXT("TRUE"), MB_OK);
		}
		else
		{
			MessageBox(hwnd, TEXT("Reflexivity FALSE"), TEXT("FALSE"), MB_OK);
		}
