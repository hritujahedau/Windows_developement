...
//		wsprintf(wszCLSID, TEXT("clsid:%s"), wszTemp);
		...
		hr = gpIOddNumber->QueryInterface(IID_IOddNumber, (void**)&gpIOddNumberAgain);
		if (FAILED(hr))
		{
			MessageBox(hwnd, TEXT("Failed to get IOddNumber again interface"), TEXT("ERROR"), MB_OK);
			DestroyWindow(hwnd);
		}

		if (gpIOddNumber == gpIOddNumberAgain)
		{
			MessageBox(hwnd, TEXT("Got same IOddNumber Interface again"), TEXT("SUCCESS"), MB_OK);
		}
		else
		{
			MessageBox(hwnd, TEXT("Failed to get same IOddNumber Interface again"), TEXT("FAILED"), MB_OK);
		}
	...
	case WM_DESTROY:
		...
		if (gpIOddNumberAgain)
		{
			gpIOddNumberAgain->Release();
			gpIOddNumberAgain = NULL;
		}
