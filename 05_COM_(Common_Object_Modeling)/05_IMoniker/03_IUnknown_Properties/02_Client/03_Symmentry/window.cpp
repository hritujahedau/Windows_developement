...
//		wsprintf(wszCLSID, TEXT("clsid:%s"), wszTemp);
		...
		hr = gpIEvenNumber->QueryInterface(IID_IOddNumber, (void**)&gpIOddNumberAgain);
		if (FAILED(hr))
		{
		...
		{
			MessageBox(hwnd, TEXT("Symmetric"), TEXT("SUCCESS"), MB_OK);
		}
		else
		{
			MessageBox(hwnd, TEXT("NOT Symmetric"), TEXT("FAILED"), MB_OK);
		}
